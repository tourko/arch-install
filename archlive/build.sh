...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages.list)" install
}
...
make_customize_airootfs() {
	...
	chown -R root:root ${work_dir}/${arch}
